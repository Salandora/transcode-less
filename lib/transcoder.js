...
var FileTranscoder = (function (_super) {
    ...
    FileTranscoder.prototype.transcode = function () {
        ...
        Fs.readFile(this.filepath, null, function (error, data) {
            if (error) {
                ...
                atom.notifications.addError(_this.relativeFilepath, { detail: error.message });
            }
            else {
    ...
    FileTranscoder.prototype.onSuccess = function (options, output) {
        ...
        Fs.writeFile(outFile, output.css, function (error) {
            if (error) {
                console.error("FileTranscoder.onSuccess(): " + error.name, error);
                atom.notifications.addError(_this.relativeFilepath, { detail: error.message });
            }
            else {
                atom.notifications.addSuccess(_this.relativeFilepath, { detail: "Rendering success" });
            }
        });
    };
    /** Make directories recurcively */
    FileTranscoder.prototype.mkdir = function (path) {
        var dirnameStack = [];
        var tempDir = path;
